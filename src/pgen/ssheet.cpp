  ...
  for (int k = kl; k <= ku; k++)
  {
    for (int j = jl; j <= ju; j++)
    {
      for (int i = il; i <= iu; i++)
      {
        ...
        if (NON_BAROTROPIC_EOS)
        {
          phydro->u(IEN, k, j, i) = rp / gm1 + 0.5 * (SQR(phydro->u(IM1, k, j, i)) + SQR(phydro->u(IM2, k, j, i)) + SQR(phydro->u(IM3, k, j, i))) / phydro->u(IDN, k, j, i);
        }
      }
    }
  }
...
void Mesh::UserWorkInLoop()
{
  ...
  if (error_output)
  {
    if ((present_time < tlim) && (nlim < 0 || cncycle < nlim) && (present_time > hst_next_time))
    {
      flag = true;
  ...
  if (flag)
  {
    ...
    for (int bn = 0; bn < nblocal; ++bn)
    {
      ...
      else if (loc.level - 1 == root_level)
      { // level difference 1
        ...
        { // 2D
          ...
          for (int j = pmb->cjs; j <= pmb->cje; j++)
          {
            for (int i = pmb->cis; i <= pmb->cie; i++)
            {
              ...
              int kk = k;
              int ti = static_cast<int>(loc.lx1 >> 1) * pmb->block_size.nx1 + static_cast<int>(loc.lx1 % 2) * (pmb->block_size.nx1 / 2) + (i - pmb->cis);
              int tj = static_cast<int>(loc.lx2 >> 1) * pmb->block_size.nx2 + static_cast<int>(loc.lx2 % 2) * (pmb->block_size.nx2 / 2) + (j - pmb->cjs);
              int tk = k;
              ...
              Real vx_vol = vx00 * vol00 + vx01 * vol01 + vx10 * vol10 + vx11 * vol11;
              Real dvy_vol = dvy00 * vol00 + dvy01 * vol01 + dvy10 * vol10 + dvy11 * vol11;
          ...
          for (int k = pmb->cks; k <= pmb->cke; k++)
          {
            for (int j = pmb->cjs; j <= pmb->cje; j++)
            {
              for (int i = pmb->cis; i <= pmb->cie; i++)
              {
                ...
                int ti = static_cast<int>(loc.lx1 >> 1) * pmb->block_size.nx1 + static_cast<int>(loc.lx1 % 2) * (pmb->block_size.nx1 / 2) + (i - pmb->cis);
                int tj = static_cast<int>(loc.lx2 >> 1) * pmb->block_size.nx2 + static_cast<int>(loc.lx2 % 2) * (pmb->block_size.nx2 / 2) + (j - pmb->cjs);
                int tk = static_cast<int>(loc.lx3 >> 1) * pmb->block_size.nx3 + static_cast<int>(loc.lx3 % 2) * (pmb->block_size.nx3 / 2) + (k - pmb->cks);
                ...
                Real SN = std::sin(kx * pmb->pcoord->x1f(ii + 1) + ky * pmb->pcoord->x2f(jj + 1));
                Real vx_vol = vx000 * vol000 + vx001 * vol001 + vx010 * vol010 + vx011 * vol011 + vx100 * vol100 + vx101 * vol101 + vx110 * vol110 + vx111 * vol111;
                Real dvy_vol = dvy000 * vol000 + dvy001 * vol001 + dvy010 * vol010 + dvy011 * vol011 + dvy100 * vol100 + dvy101 * vol101 + dvy110 * vol110 + dvy111 * vol111;
                vxs(tk, tj, ti) = 2.0 * SN * vx_vol;
                dvys(tk, tj, ti) = 2.0 * SN * dvy_vol;
              }
            }
...
namespace
{
  ...
  Real Historydvyc(MeshBlock *pmb, int iout)
  {
    ...
    Real dvy0 = iso_cs * amp * std::fabs(static_cast<Real>(nwy) / static_cast<Real>(nwx));
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k)
  {
    ...
    for (int j = pmb->js; j <= pmb->je; ++j)
    {
      ...
      for (int i = pmb->is; i <= pmb->ie; ++i)
      {
        ...
        x1 = pmb->pcoord->x1v(i);
        // m component
        cons(IM2, k, j, i) += dt * Sig * A * m * std::sin(m * x2);
      }
    }
  }
}